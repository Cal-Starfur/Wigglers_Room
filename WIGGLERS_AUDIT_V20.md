...
**Fix:** Extract all five as module-level helpers (above both `drawFarmerSnoo` and `drawSnoo`). Remove the inner copies from both functions. Both callers share the same helpers.
...
|---|---|---|---|
| 1 | Extract duplicate `svgX/svgY/makeSnooGrad/makeIrisGrad/smilePath` to shared helpers | Low | Removes 5 duplicate functions + 5 copy-paste blocks |
| 2 | Route all raw `localStorage.setItem` calls through `saveSession()` | Low | Fixes Devvit persistence for flood/death/drain events |
| 3 | Add `MSG_*` named constants to `game.js` matching `main.tsx` | Low | Devvit compliance, prevents silent typo failures |
...
| 7 | Tag `nearestPathIdx` as reserved for NPC worms | Trivial | Prevents future accidental deletion |
| 8 | Rename `_underscore` functions to camelCase | Medium | Convention compliance |
| 9 | Split the 5 longest functions | High effort | Maintainability — do when touching those systems |
| 10 | Convert `var` loop variables to `let` | Low (risky) | Scope safety — low priority, high change volume |
