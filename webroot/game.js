...
window.addEventListener('message', function(e) {
  ...
  // Devvit host sends saved session from KV store on post open.
  // Shape is identical to the localStorage session object.
  // { type: 'setSession', session: { ts, karma, pEaten, pSR, pSEG,
  ...
  if (msg.type === 'claimWorm') {
    ...
    if (msg.generation != null) generation = msg.generation;
    var _claimX = msg.x != null ? msg.x : W/2;
    var _claimY = msg.y != null ? msg.y : (2.1 * H);
    pSegs = []; pHist = [];
    for (var _ci = 0; _ci < 20; _ci++) pHist.push({x: _claimX, y: _claimY});
    for (var _si = 0; _si < Math.max(4, pSEG); _si++) {
      pSegs.push({x: _claimX - _si * pSR * 2, y: _claimY});
    }
    mX = _claimX; mY = _claimY;
...
function drawFarmerSnoo(ctx, sx, sy, lidAng, buckAng, scene) {
  ...
  ctx.beginPath();
  ctx.ellipse(0, torsoBot + legH + bootH*0.3, bodyW*0.85, sc*0.025, 0, 0, Math.PI*2);
  ctx.fill();

  // ── Legs — jeans ─────────────────────────────────────────────────────────
  var lg = ctx.createLinearGradient(-bodyW*0.55, torsoBot, bodyW*0.0, torsoBot+legH);
  lg.addColorStop(0, '#5a8ad8'); lg.addColorStop(0.5,'#4a78c4'); lg.addColorStop(1,'#3a68b0');
  ctx.fillStyle = lg;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.42, torsoBot, bodyW*0.34, legH, [0,0,4,4]); ctx.fill();
  var rg = ctx.createLinearGradient(bodyW*0.0, torsoBot, bodyW*0.55, torsoBot+legH);
  ...
  // ── Boots ─────────────────────────────────────────────────────────────────
  function drawBoot(bx) {
    var by = torsoBot + legH;
    ctx.fillStyle = '#6a3a18';
    ctx.beginPath(); ctx.ellipse(bx, by, bootW, bootH, 0, 0, Math.PI); ctx.fill();
    ctx.fillStyle = '#8a5028';
    ctx.beginPath(); ctx.ellipse(bx - bootW*0.08, by - bootH*0.35, bootW*0.58, bootH*0.32, -0.1, 0, Math.PI); ctx.fill();
  ...
  // ── Left arm UPPER — drawn behind body ───────────────────────────────────
  ctx.save();
  ctx.translate(-bodyW*0.50, torsoTop + bodyH*0.10);
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = sc*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-armLen*0.48, armLen*0.52); ctx.stroke();
  ctx.restore();

  // ── Right arm — hangs at side (no longer raises to lift lid) ────────────
  ctx.save();
  ctx.translate(bodyW*0.50, torsoTop + bodyH*0.10);
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = sc*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(armLen*0.48, armLen*0.52); ctx.stroke();
  ctx.fillStyle = '#f0f0f0';
  ctx.beginPath(); ctx.arc(armLen*0.48, armLen*0.52, handR, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#c8c8c8'; ctx.lineWidth = sc*0.008;
  ctx.beginPath(); ctx.arc(armLen*0.48, armLen*0.52, handR, 0, Math.PI*2); ctx.stroke();
  ctx.restore();

  // ── Flannel shirt sleeves ─────────────────────────────────────────────────
  ctx.fillStyle = '#b82820';
  ctx.beginPath(); ctx.roundRect(-bodyW*0.60, torsoTop+bodyH*0.06, bodyW*0.18, bodyH*0.58, 6); ctx.fill();
  ctx.beginPath(); ctx.roundRect( bodyW*0.42, torsoTop+bodyH*0.06, bodyW*0.18, bodyH*0.58, 6); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,0.12)'; ctx.lineWidth = sc*0.008;
  for (var fl = 0; fl < 3; fl++) {
    var fpx = -bodyW*0.62 + fl*bodyW*0.05;
    ctx.beginPath(); ctx.moveTo(fpx, torsoTop+bodyH*0.10); ctx.lineTo(fpx, torsoTop+bodyH*0.60); ctx.stroke();
  }
  for (var fl2 = 0; fl2 < 3; fl2++) {
    var fpx2 = bodyW*0.40 + fl2*bodyW*0.05;
    ctx.beginPath(); ctx.moveTo(fpx2, torsoTop+bodyH*0.10); ctx.lineTo(fpx2, torsoTop+bodyH*0.60); ctx.stroke();
  }

  // ── Dungarees — bib and body ──────────────────────────────────────────────
  var dg = ctx.createLinearGradient(-bodyW*0.50, torsoTop, bodyW*0.50, torsoBot);
  dg.addColorStop(0, '#5a8ad8'); dg.addColorStop(1, '#3a68b0');
  ctx.fillStyle = dg;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.50, torsoTop, bodyW*1.00, bodyH, 8); ctx.fill();
  ctx.fillStyle = '#b82820';
  ctx.beginPath(); ctx.roundRect(-bodyW*0.50, torsoTop+bodyH*0.38, bodyW*1.00, bodyH*0.62, [0,0,8,8]); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,0.10)'; ctx.lineWidth = sc*0.008;
  for (var pc = 0; pc < 6; pc++) {
    var ppx = -bodyW*0.45 + pc*bodyW*0.16;
    ctx.beginPath(); ctx.moveTo(ppx, torsoTop+bodyH*0.42); ctx.lineTo(ppx, torsoTop+bodyH*0.98); ctx.stroke();
  }
  ctx.strokeStyle = 'rgba(255,255,255,0.07)'; ctx.lineWidth = sc*0.010;
  for (var pr = 0; pr < 3; pr++) {
    var pry = torsoTop+bodyH*0.50 + pr*bodyH*0.16;
    ctx.beginPath(); ctx.moveTo(-bodyW*0.45, pry); ctx.lineTo(bodyW*0.45, pry); ctx.stroke();
  }
  // Bib pocket
  var bibG = ctx.createLinearGradient(-bodyW*0.26, torsoTop+bodyH*0.03, bodyW*0.26, torsoTop+bodyH*0.43);
  bibG.addColorStop(0,'#5e8ee0'); bibG.addColorStop(1,'#3a70ba');
  ctx.fillStyle = bibG;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.26, torsoTop+bodyH*0.03, bodyW*0.52, bodyH*0.40, 5); ctx.fill();
  ctx.strokeStyle = '#6a9ade'; ctx.lineWidth = sc*0.007;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.22, torsoTop+bodyH*0.07, bodyW*0.44, bodyH*0.30, 3); ctx.stroke();
  // Straps
  ctx.strokeStyle = '#4a78c4'; ctx.lineWidth = sc*0.032; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(-bodyW*0.20, torsoTop); ctx.lineTo(-bodyW*0.13, headCY+headR*0.80); ctx.stroke();
  ctx.beginPath(); ctx.moveTo( bodyW*0.20, torsoTop); ctx.lineTo( bodyW*0.13, headCY+headR*0.80); ctx.stroke();
  // Brass buttons
  ctx.fillStyle = '#d4a820';
  ctx.beginPath(); ctx.arc(-bodyW*0.165, torsoTop+bodyH*0.03, sc*0.012, 0, Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.arc( bodyW*0.165, torsoTop+bodyH*0.03, sc*0.012, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#a07810'; ctx.lineWidth = sc*0.005;
  ctx.beginPath(); ctx.arc(-bodyW*0.165, torsoTop+bodyH*0.03, sc*0.012, 0, Math.PI*2); ctx.stroke();
  ctx.beginPath(); ctx.arc( bodyW*0.165, torsoTop+bodyH*0.03, sc*0.012, 0, Math.PI*2); ctx.stroke();

  // ── Left arm FOREARM + BASKET — drawn in front of body ───────────────────
  ctx.save();
  ctx.translate(-bodyW*0.50, torsoTop + bodyH*0.10);
  ctx.translate(-armLen*0.48, armLen*0.52);
  ctx.rotate(buckAng);
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = sc*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(-armLen*0.40, armLen*0.45); ctx.stroke();
  ctx.translate(-armLen*0.40, armLen*0.45);
  ctx.fillStyle = '#f0f0f0';
  ctx.beginPath(); ctx.arc(0, 0, handR, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#c8c8c8'; ctx.lineWidth = sc*0.008;
  ctx.beginPath(); ctx.arc(0, 0, handR, 0, Math.PI*2); ctx.stroke();
  // Basket
  var bkW = sc*0.095; var bkH = sc*0.108;
  // Items — drawn before basket body, tops peeking above the rim
  if (buckAng < Math.PI*0.45 && typeof snooBucketItems !== 'undefined' && snooBucketItems.length) {
    var bkPos = [
      {x:-bkW*0.32, y:-bkH*0.48, r:bkW*0.30, rot: 0.4},
      {x: bkW*0.25, y:-bkH*0.52, r:bkW*0.26, rot:-0.7},
      {x:-bkW*0.08, y:-bkH*0.58, r:bkW*0.28, rot: 1.1},
      {x: bkW*0.40, y:-bkH*0.44, r:bkW*0.22, rot:-0.3},
    ];
    for (var bi5 = 0; bi5 < bkPos.length && bi5 < snooBucketItems.length; bi5++) {
      var bip = bkPos[bi5]; var bit = snooBucketItems[bi5];
      ctx.save(); ctx.translate(bip.x, bip.y); ctx.rotate(bip.rot);
      drawDebrisFragment(ctx, bit.name, bip.r, bit.debrisCol, bit.debrisCol2);
      ctx.restore();
    }
  }
  // Basket body
  var bkg = ctx.createLinearGradient(-bkW, -bkH*0.4, bkW, bkH*0.5);
  bkg.addColorStop(0, '#c8902a'); bkg.addColorStop(0.5,'#a87020'); bkg.addColorStop(1,'#8a5810');
  ctx.fillStyle = bkg;
  ctx.beginPath();
  ctx.moveTo(-bkW, -bkH*0.40);
  ctx.lineTo(-bkW*0.80, bkH*0.55);
  ctx.quadraticCurveTo(0, bkH*0.68, bkW*0.80, bkH*0.55);
  ctx.lineTo(bkW, -bkH*0.40);
  ctx.closePath(); ctx.fill();
  // Weave lines over items
  ctx.strokeStyle = 'rgba(60,30,0,0.25)'; ctx.lineWidth = sc*0.009;
  for (var wl = 0; wl < 4; wl++) {
    var wy2 = -bkH*0.30 + wl * bkH*0.22;
    ctx.beginPath(); ctx.moveTo(-bkW*(0.75+wl*0.05), wy2); ctx.lineTo(bkW*(0.75+wl*0.05), wy2); ctx.stroke();
  }
  var rimG = ctx.createLinearGradient(-bkW, 0, bkW, 0);
  rimG.addColorStop(0,'#d4a030'); rimG.addColorStop(0.5,'#f0c050'); rimG.addColorStop(1,'#c49020');
  ctx.fillStyle = rimG;
  ctx.beginPath(); ctx.roundRect(-bkW, -bkH*0.44, bkW*2, sc*0.022, 3); ctx.fill();
  ctx.strokeStyle = '#8a5810'; ctx.lineWidth = sc*0.018; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.arc(0, -bkH*0.40, bkW*0.52, Math.PI, 0); ctx.stroke();
  ctx.restore(); // forearm+basket
  ...
  // Pupil shine — bright lower oval
  ctx.fillStyle = '#FF6101';
  ctx.beginPath(); ctx.ellipse(svgX(88.0),  svgY(149.1), 9.3*S, 7.1*S, 0, 0, Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(svgX(168.2), svgY(149.1), 9.3*S, 7.1*S, 0, 0, Math.PI*2); ctx.fill();

  // White catchlight
  ctx.fillStyle = 'rgba(255,255,255,0.92)';
  ctx.beginPath(); ctx.arc(svgX(82),  svgY(132), 4.8*S, 0, Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.arc(svgX(162), svgY(132), 4.8*S, 0, Math.PI*2); ctx.fill();

  // Nose dots
  ctx.fillStyle = '#FFC49C';
  ctx.beginPath(); ctx.ellipse(svgX(94.4), svgY(134.8),3.3*S,3.6*S,0,0,Math.PI*2); ctx.fill();
  ...
  // Head-top patch — blend stem base into head
  ctx.save();
  ctx.beginPath();
...
// ── Update Snoo cinematic state machine ───────────────────────────────────
function updateSnoo() {
  if (!snooScene) return;
  ...
  var b = getBin();
  // Snoo is now drain-sized — stands just above the bin top, slides in from the left.
  // sc3 must match drawFarmerSnoo sc = H * 0.16 exactly.
  var sc3 = H * 0.16;
  var lidSY3 = H * 0.5 - camY;  // screen Y of the bin lid
  // Position torso top so boots land just above the lid line
  var bodyH3  = sc3 * 0.270;
  var legH3   = sc3 * 0.225;
  var bootH3  = sc3 * 0.058;
  var snooSY = lidSY3 - bodyH3 - legH3 - bootH3 * 0.5;
  snooTargetX = b.cx + b.bw2 * 0.05;

  // Track bucket mouth screen position — mirrors drawFarmerSnoo left arm geometry exactly.
  // Left arm: shoulder at (-bodyW*0.50, torsoTop+bodyH*0.10)
  //           upper arm to (-armLen*0.48, armLen*0.52) from shoulder
  //           forearm rotated by buckAng then moves (-armLen*0.40, armLen*0.45)
  var armLen3 = sc3 * 0.165;
  var bodyW3  = sc3 * 0.195;
  var shoulderX = snooX - bodyW3 * 0.50;
  var shoulderY = snooSY + bodyH3 * 0.10;
  var elbowX = shoulderX - armLen3 * 0.48;
  var elbowY = shoulderY + armLen3 * 0.52;
  var foreAng = snooBucketAngle;
  ...
  switch(snooPhase) {
    ...
    case 'bend':
      snooScale = 1;
      // Lid swings open on its own as Snoo arrives — right arm stays at side
      var bendT = Math.min(1, snooT / snooPhaseDur.bend);
      snooLidAngle = bendT * Math.PI * 0.55;
      snooBucketAngle = 0;
      if (snooT >= snooPhaseDur.bend) { snooPhase = 'tip'; snooT = 0; }
      break;

    case 'tip':
      snooScale = 1;
      // Clear bucket items once tipped past halfway
      if (snooBucketAngle >= Math.PI*0.45) snooBucketItems = [];
      var tipT = Math.min(1, snooT / snooPhaseDur.tip);
      snooLidAngle = Math.PI * 0.55;
      // Halfway through the tip — fire scene-specific payloads
      if (snooT === Math.floor(snooPhaseDur.tip * 0.5)) {
        if (snooScene === 'feed' || snooScene === 'emergency') {
          // Scraps refill — Snoo pours food into the bin
          snooScrapsCascade = true;
          snooCascadeT = 0;
          trashChunks = []; debris = []; bugs = [];
          scrapsLevel = 1.0; scrapsEmpty = false; tapReady = false;
          spawnScraps();
          if (snooScene === 'emergency') { emergencyKarmaPot = 0; emergencyRequested = false; emergencyCooldown = 300; }
        }
        // TODO: 'drain' scene payload goes here once new drain animation is built
      }
      if (snooT >= snooPhaseDur.tip) { snooPhase = 'straighten'; snooT = 0; }
      break;

    case 'straighten':
      snooScale = 1;
      // Bucket returns upright after tipping
      var strT = Math.min(1, snooT / snooPhaseDur.straighten);
      snooLidAngle = Math.PI * 0.55 * (1 - strT);
      if (snooT >= snooPhaseDur.straighten) { snooPhase = 'slideout'; snooT = 0; }
      break;
  ...
  if (snooScrapsCascade || snooCascadeT > 0) {
    for (var di = 0; di < trashChunks.length; di++) {
      var dc = trashChunks[di];
      if (!dc.dropping) continue;
      dc.dropY += dc.dropVy;
      dc.dropVy += 0.18; // gravity
      dc.dropX = (dc.dropX !== undefined ? dc.dropX : dc.x) + (dc.dropVx || 0);
      dc.rot += dc.rotSpd * 4;
      if (dc.dropY >= dc.y) {
        dc.dropY = dc.y;
        dc.dropX = dc.x;
  ...
  // Cascade particle counter
  if (snooScrapsCascade) {
    snooCascadeT++;
    if (snooCascadeT > 60) snooScrapsCascade = false;
  }
}

// ── Trigger the Snoo cinematic ────────────────────────────────────────────
function triggerSnoo(scene) {
  if (snooScene) return; // already running
  snooScene = scene;
  snooPhase = 'slidein';
  snooT = 0;
  snooX = -W * 0.15;
  ...
  snooCamPrev = camY;
  snooScale = 0;
  // Pick 4 random real trash types for the bucket
  var shuffled = TRASH_TYPES.slice().sort(function(){ return Math.random()-0.5; });
  snooBucketItems = shuffled.slice(0, 4);
}

// ── Trigger the drain cinematic ───────────────────────────────────────────
function triggerSnooDrain() {
  if (drainScene || snooScene) return; // don't double-trigger
  if (tLvl <= 0) { triggerWeeklyDrain(); return; } // nothing to drain, just reset

  drainScene       = true;
  drainPhase       = 'floatin';
  drainPT          = 0;
  drainTapRot      = 0;
  drainTapIsOpen   = false;
  drainTeaFlow     = 0;
  drainBucketFill  = 0;
  drainVisualTLvl  = tLvl;
  drainSnooScale   = 1;
  drainToastAlpha  = 0;
  // Bonus pool: same formula as triggerWeeklyDrain — calculated NOW so toast shows correct value
  drainBonusPool   = weeklyContrib > 0 ? Math.round(tLvl * 2000) : 0;
  // Drain duration: 60 frames per 0.1 of tLvl, min 80, max 220
...
function updateSnooDrain() {
  ...
  switch (drainPhase) {
    case 'floatin':
      var t0 = Math.min(drainPT / DRAIN_PHASE_DUR.floatin, 1);
      drainSnooScale = 1;
      drainSnooX = -W * 0.15 + (STOP_X + W * 0.15) * easeOut(t0);
      drainSnooY = STOP_Y;
      if (drainPT >= DRAIN_PHASE_DUR.floatin) {
        drainSnooX = STOP_X; drainSnooY = STOP_Y;
        drainPhase = 'pause'; drainPT = 0;
      }
      break;

    case 'pause':
      drainSnooScale = 1;
      if (drainPT >= DRAIN_PHASE_DUR.pause) { drainPhase = 'openvalve'; drainPT = 0; }
      break;

    case 'openvalve':
      drainSnooScale = 1;
      var t1 = Math.min(drainPT / DRAIN_PHASE_DUR.openvalve, 1);
      drainTapRot = t1 * Math.PI * 0.82;
      if (drainPT > 18) {
        drainTapIsOpen = true;
        drainTeaFlow   = Math.min((drainPT - 18) / 18, 1);
      }
      if (drainPT >= DRAIN_PHASE_DUR.openvalve) { drainPhase = 'draining'; drainPT = 0; }
      break;

    case 'draining':
      drainSnooScale = 1;
      drainTeaFlow   = 1;
      var drainFrac  = drainPT / drainDrainingDur;
      // Drain real tLvl proportionally so the sump visual empties in sync
      tLvl = Math.max(0, drainVisualTLvl * (1 - drainFrac));
      // Bucket fills as sump empties
      drainBucketFill = Math.min(drainFrac, 1);
      if (drainPT >= drainDrainingDur) {
        tLvl = 0;
        drainBucketFill = 1;
        drainPhase = 'closevalve'; drainPT = 0;
      }
      break;

    case 'closevalve':
      drainSnooScale = 1;
      var t2 = Math.min(drainPT / DRAIN_PHASE_DUR.closevalve, 1);
      drainTapRot    = Math.PI * 0.82 * (1 - t2);
      drainTeaFlow   = Math.max(0, 1 - t2 * 2);
      drainToastAlpha = Math.min(t2 * 2, 1);
      if (drainPT >= DRAIN_PHASE_DUR.closevalve) {
        drainTapIsOpen  = false;
        drainTeaFlow    = 0;
        drainTapRot     = 0;
        drainPhase = 'floatout'; drainPT = 0;
      }
      break;

    case 'floatout':
      var t3 = Math.min(drainPT / DRAIN_PHASE_DUR.floatout, 1);
      drainSnooScale  = 1;
      drainSnooX      = STOP_X + (-W * 0.15 - STOP_X) * easeIn(t3);
      drainSnooY      = STOP_Y;
      drainToastAlpha = Math.max(0, 1 - t3);
      if (drainPT >= DRAIN_PHASE_DUR.floatout) {
        drainScene     = false;
        drainPhase     = 'done';
        snooGamePaused = false;
        drainOwner     = null; // release tLvl ownership before triggerWeeklyDrain runs
        // Fire weekly drain now — popup appears over the worm immediately after Snoo leaves
        triggerWeeklyDrain();
      }
      break;
  }
...
function drawSnooDrain() {
  ...
  // Right arm carry pose — hand holds bucket under tap
  var ux = armLen * 0.10, uy = armLen * 0.50;
  var fx = ux + armLen * 0.08, fy = uy + armLen * 0.42;
  var handWX = sx + bodyW * 0.50 + fx;
  var handWY = sy + torsoTop + bodyH * 0.10 + fy;

  // Left arm — hangs at side, slightly forward (tap is too far above to reach at this scale)
  var lShoulderX = sx - bodyW * 0.50;
  var lShoulderY = sy + torsoTop + bodyH * 0.10;
  var elbowX = lShoulderX - armLen * 0.28;
  var elbowY = lShoulderY + armLen * 0.50;
  var lHandX = elbowX - armLen * 0.18;
  var lHandY = elbowY + armLen * 0.42;
  ...
  ctx.fillStyle = 'rgba(0,0,0,0.18)';
  ctx.beginPath();
  ctx.ellipse(sx, sy + torsoBot + legH + bootH*0.3, bodyW*0.85, SC*0.025, 0, 0, Math.PI*2);
  ctx.fill();

  // ── Legs ─────────────────────────────────────────────────────────────────
  ctx.save(); ctx.translate(sx, sy);
  var lg = ctx.createLinearGradient(-bodyW*0.55, torsoBot, 0, torsoBot+legH);
  lg.addColorStop(0,'#5a8ad8'); lg.addColorStop(0.5,'#4a78c4'); lg.addColorStop(1,'#3a68b0');
  ctx.fillStyle = lg;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.42, torsoBot, bodyW*0.34, legH, [0,0,4,4]); ctx.fill();
  var rg = ctx.createLinearGradient(0, torsoBot, bodyW*0.55, torsoBot+legH);
  rg.addColorStop(0,'#5a8ad8'); rg.addColorStop(0.5,'#4a78c4'); rg.addColorStop(1,'#3a68b0');
  ctx.fillStyle = rg;
  ctx.beginPath(); ctx.roundRect(bodyW*0.08, torsoBot, bodyW*0.34, legH, [0,0,4,4]); ctx.fill();
  // Boots
  ...
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = SC*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(lShoulderX, lShoulderY); ctx.lineTo(elbowX, elbowY); ctx.stroke();

  // ── Right arm carry pose ──────────────────────────────────────────────────
  ctx.save(); ctx.translate(sx + bodyW*0.50, sy + torsoTop + bodyH*0.10);
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = SC*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(ux,uy); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(ux,uy); ctx.lineTo(fx,fy); ctx.stroke();
  ctx.fillStyle = '#f0f0f0'; ctx.beginPath(); ctx.arc(fx,fy,handR,0,Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#c8c8c8'; ctx.lineWidth = SC*0.008;
  ctx.beginPath(); ctx.arc(fx,fy,handR,0,Math.PI*2); ctx.stroke();
  ctx.restore();
  ...
  ctx.strokeStyle = 'rgba(255,255,255,0.12)'; ctx.lineWidth = SC*0.008;
  for (var fl=0; fl<3; fl++) {
    ctx.beginPath(); ctx.moveTo(-bodyW*0.62+fl*bodyW*0.05, torsoTop+bodyH*0.10); ctx.lineTo(-bodyW*0.62+fl*bodyW*0.05, torsoTop+bodyH*0.60); ctx.stroke();
    ctx.beginPath(); ctx.moveTo( bodyW*0.40+fl*bodyW*0.05, torsoTop+bodyH*0.10); ctx.lineTo( bodyW*0.40+fl*bodyW*0.05, torsoTop+bodyH*0.60); ctx.stroke();
  }

  // ── Dungarees ─────────────────────────────────────────────────────────────
  var dg = ctx.createLinearGradient(-bodyW*0.50, torsoTop, bodyW*0.50, torsoBot);
  dg.addColorStop(0,'#5a8ad8'); dg.addColorStop(1,'#3a68b0');
  ctx.fillStyle = dg; ctx.beginPath(); ctx.roundRect(-bodyW*0.50, torsoTop, bodyW, bodyH, 8); ctx.fill();
  ctx.fillStyle = '#b82820';
  ctx.beginPath(); ctx.roundRect(-bodyW*0.50, torsoTop+bodyH*0.38, bodyW, bodyH*0.62, [0,0,8,8]); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,0.10)'; ctx.lineWidth = SC*0.008;
  for (var pc=0; pc<6; pc++) {
    var ppx = -bodyW*0.45 + pc*bodyW*0.16;
    ctx.beginPath(); ctx.moveTo(ppx, torsoTop+bodyH*0.42); ctx.lineTo(ppx, torsoTop+bodyH*0.98); ctx.stroke();
  ...
  var bibG = ctx.createLinearGradient(-bodyW*0.26, torsoTop+bodyH*0.03, bodyW*0.26, torsoTop+bodyH*0.43);
  bibG.addColorStop(0,'#5e8ee0'); bibG.addColorStop(1,'#3a70ba');
  ctx.fillStyle = bibG; ctx.beginPath(); ctx.roundRect(-bodyW*0.26, torsoTop+bodyH*0.03, bodyW*0.52, bodyH*0.40, 5); ctx.fill();
  ctx.strokeStyle = '#6a9ade'; ctx.lineWidth = SC*0.007;
  ctx.beginPath(); ctx.roundRect(-bodyW*0.22, torsoTop+bodyH*0.07, bodyW*0.44, bodyH*0.30, 3); ctx.stroke();
  // Straps
  ctx.strokeStyle = '#4a78c4'; ctx.lineWidth = SC*0.032; ctx.lineCap = 'round';
  ...
  // ── Left forearm + hand reaching to tap ──────────────────────────────────
  ctx.strokeStyle = '#c03028'; ctx.lineWidth = SC*0.065; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(elbowX, elbowY); ctx.lineTo(lHandX, lHandY); ctx.stroke();
  ctx.fillStyle = '#f0f0f0'; ctx.beginPath(); ctx.arc(lHandX, lHandY, handR, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#c8c8c8'; ctx.lineWidth = SC*0.008;
  ctx.beginPath(); ctx.arc(lHandX, lHandY, handR, 0, Math.PI*2); ctx.stroke();
  ...
  // ── Tea drips — fall straight down from spout into bucket mouth ──────────
  ctx.save();
  if (drainTapIsOpen && drainTeaFlow > 0) {
    var spoutX = TAP_SX + 1;
    var spoutY = TAP_SY + 22;
    var bucketMouthX = handWX;
    var bucketMouthY = handWY;
    ctx.globalAlpha = drainTeaFlow * 0.92;
    var NDRIPS = 9;
    for (var di=0; di<NDRIPS; di++) {
      var t = ((di / NDRIPS) + (frame * 0.045)) % 1;
      // Straight down with a slight horizontal wobble
      var bx2 = spoutX + (bucketMouthX - spoutX) * t + Math.sin(frame*0.3 + di*1.2) * 1.2;
      var by2 = spoutY + (bucketMouthY - spoutY) * t;
      var dr = 1.8 + t * 1.6;
      ctx.fillStyle = '#6ab030';
      ctx.beginPath(); ctx.ellipse(bx2, by2, dr, dr*1.3, 0, 0, Math.PI*2); ctx.fill();
    }
    ctx.globalAlpha = 1;
  }
  ctx.restore();

  // ── Bucket held in right hand ─────────────────────────────────────────────
  ctx.save();
  ctx.translate(handWX - BK_W*0.5, handWY);
  ctx.fillStyle = '#6a5028';
  ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(BK_W,0); ctx.lineTo(BK_W*0.85,BK_H); ctx.lineTo(BK_W*0.15,BK_H); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = '#8a7040'; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.strokeStyle = '#5a4018'; ctx.lineWidth = 1.2;
  ctx.beginPath(); ctx.moveTo(BK_W*0.15,0); ctx.quadraticCurveTo(BK_W*0.5,-BK_H*0.55,BK_W*0.85,0); ctx.stroke();
  var rimG = ctx.createLinearGradient(0,-4,BK_W,-4);
  rimG.addColorStop(0,'#d4a030'); rimG.addColorStop(0.5,'#f0c050'); rimG.addColorStop(1,'#c49020');
  ctx.fillStyle = rimG; ctx.beginPath(); ctx.roundRect(-1,-4,BK_W+2,7,2); ctx.fill();
  if (drainBucketFill > 0) {
    var fh  = BK_H * 0.88 * drainBucketFill;
    var tl2 = BK_W*0.15 + (BK_W*0.70)*(1-drainBucketFill)*0.5;
    var tr2 = BK_W*0.85 - (BK_W*0.70)*(1-drainBucketFill)*0.5;
    var teaFillG = ctx.createLinearGradient(0, BK_H-fh, 0, BK_H);
    teaFillG.addColorStop(0,'rgba(120,200,40,0.70)'); teaFillG.addColorStop(1,'rgba(70,140,20,0.95)');
    ctx.fillStyle = teaFillG; ctx.globalAlpha = 0.90;
    ctx.beginPath(); ctx.moveTo(tl2,BK_H-fh); ctx.lineTo(tr2,BK_H-fh); ctx.lineTo(BK_W*0.85,BK_H); ctx.lineTo(BK_W*0.15,BK_H); ctx.closePath(); ctx.fill();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = 'rgba(160,240,80,0.55)'; ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.moveTo(tl2,BK_H-fh); ctx.lineTo(tr2,BK_H-fh); ctx.stroke();
  }
  ctx.restore();
  ...
  if (drainToastAlpha > 0) {
    ...
    var toastY = bsy - H*0.08;
    ctx.fillStyle = 'rgba(0,0,0,0.72)';
    ctx.beginPath(); ctx.roundRect(W/2-105, toastY-32, 210, 48, 10); ctx.fill();
    ctx.fillStyle = '#7ec830'; ctx.font = 'bold ' + Math.round(H*0.022) + 'px monospace';
...
function drawSnooCinematic() {
  if (!snooScene) return; // nothing to draw — snooScene is always null when phase is 'done'

  var b = getBin();

  // Scene label — small pill at top
  var sceneLabel = snooScene === 'feed' ? '🪣 Farmer Snoo refills the bin!' : '🚨 Emergency delivery';
  ctx.fillStyle = 'rgba(20,40,15,0.88)';
  ctx.beginPath(); ctx.roundRect(W/2 - 90, 8, 180, 22, 8); ctx.fill();
  ctx.fillStyle = '#a0e880';
  ctx.font = 'bold 11px sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText(sceneLabel, W/2, 23);
}
...
function setup() {
  // Stop the canvas preview animation — game is taking over the canvas
...
function updatePhysics() {
  ...
  // ── Weekly drain check ────────────────────────────────────────────────────
  var nowW = Date.now();
  if (weekStartTs === 0) weekStartTs = nowW;
  if (nowW - weekStartTs >= WEEK_DRAIN_MS && !drainScene && drainOwner !== "valve") {
    // Guard: do not fire while player valve is open or a cinematic is already running.
...
function triggerWeeklyDrain() {
  ...
  if (_tLvlForBonus <= 0) { weekStartTs = Date.now(); weeklyContrib = 0; return; }

  // Karma pool based on tea level — full sump = big bonus
  var bonusPool = Math.round(_tLvlForBonus * 2000);
  var share = weeklyContrib > 0 ? bonusPool : 0;
  ...
  drainOwner = null; // release ownership — drain cycle fully complete

  window._drainMsg = '🌿 Farmer drained the tea! Bonus shared: +' + bonusPool;
...
function draw() {
  ...
  if (snooScene) {
    var lidSY = H * 0.5 - camY;
    var sc16 = H * 0.16;
    var bodyH16 = sc16 * 0.270;
    var legH16  = sc16 * 0.225;
    var bootH16 = sc16 * 0.058;
    // Boots land just above the lid line
    var snooDrawSY = lidSY - bodyH16 - legH16 - bootH16 * 0.5;
    ctx.save();
    drawFarmerSnoo(ctx, snooX, snooDrawSY, snooLidAngle, snooBucketAngle, snooScene);
    ctx.restore();
  }
  ...
  // --- Tier 0 pile: dark soil fill + straw blanket, drawn before trash so items sit in it ---
  {
    ...
    if (bkT0Bot > 0 && bkT0Bot < H * 2) {
      ...
      if (bkPH > 0 && bkPSY < H) {
        // Dark soil fill between trash objects
        var bkPGrad = ctx.createLinearGradient(0, bkPSY, 0, bkT0Bot);
        bkPGrad.addColorStop(0,   '#1e1208');
        bkPGrad.addColorStop(0.5, '#180e06');
        bkPGrad.addColorStop(1,   '#120a04');
        ctx.fillStyle = bkPGrad;
        ctx.fillRect(bkb.cx-bkb.bw2, bkPSY, bkb.bw, bkPH);
...
function loop() {
  ...
  try { updateSnooDrain(); } catch(e) { showErr('updateSnooDrain: '+e.message); }
...
root.addEventListener('click', function(e) {
  ...
  if (!deathScreen && window._valveBtn) {
    ...
    if (cx >= db.x && cx <= db.x+db.w && cy >= db.y && cy <= db.y+db.h) {
      if (tapReady && drainTapCooldown <= 0) {
        // Weekly feed tap — same spigot, different action
        triggerSnoo('feed');
        tapReady = false;
        return;
      }
  ...
  if (window._emergencyDonateBtn && !deathScreen) {
    ...
    if (cx >= eb.x && cx <= eb.x+eb.w && cy >= eb.y && cy <= eb.y+eb.h && karma >= 5 && emergencyCooldown <= 0) {
      ...
      if (emergencyKarmaPot >= EMERGENCY_KARMA_NEEDED) {
        triggerSnoo('emergency');
      }
      return;
    }
  }
...
root.addEventListener('touchstart', function(e) {
  ...
  // ── Tap-ready Snoo trigger (single finger only) ─────────────────────────
  if (tapReady && n === 1 && drainTapCooldown <= 0) {
    tapReady = false;
    triggerSnoo('feed');   // feed refill only — weekly drain stays on its own timer
    return;
  }
    ...
    if (peak === 1 && _gesture.swipeLive) {
      ...
      if (isTap && window._valveBtn) {
        ...
        if (ex0 >= db2.x && ex0 <= db2.x+db2.w && ey0 >= db2.y && ey0 <= db2.y+db2.h) {
          if (tapReady && drainTapCooldown <= 0) {
            triggerSnoo('feed'); tapReady = false;
          } else if (floodActive || valveOpen) {
            if (!valveOpen && drainTapCooldown <= 0) {
              triggerDrainTap();
            } else if (valveOpen) {
              closeDrainTap();
...
window.addEventListener('keydown', function(e) {
  ...
  // DEBUG — W key triggers Snoo feed cinematic (scraps refill).
  if (e.code === 'KeyW') { e.preventDefault(); triggerSnoo('feed'); }
  // DEBUG — T key triggers Snoo drain cinematic.
  if (e.code === 'KeyT') {
    e.preventDefault();
    tLvl = 0.8;
...
// ── Preview mode — canvas animation before game init ──────────────────────────
// Runs immediately when game.js loads. Uses requestAnimationFrame so the canvas
// animates at 60fps with zero URL churn — eliminating the Chrome hover-flicker.
// stopPreview() is called from setup() when the game proper begins.
//
// Uses the same drawTrashChunk() function that game.js already has, so no shapes
// need to be duplicated. The layout mirrors the 33-item TRASH_LAYOUT from the
// previous main.tsx SVG animation, ported directly to canvas coords.
//
// Canvas starts at 0x0 and resizeCanvas() hasn't run yet, so we size it here.
(function() {
  var _pvActive = true;
  var _pvT = 0;     // animation time in seconds
  var _pvLast = 0;  // last timestamp for delta

  // Item layout — same positions, sizes, rotations as the SVG version
  // x/y are in 512-unit space; we scale to actual canvas size each frame.
  var _pvItems = [
    ...
    { name: 'watermelon_chunk',x: 194, yOff: 462, r: 17, rot: -0.62 },
    ...
    { name: 'watermelon_chunk',x: 238, yOff:  30, r: 22, rot: -0.62 },
  ...
  var _pvTileH = 512;
  var _pvFallPx = 20; // px/s in 512-unit space

  function _pvSizeCanvas() {
    var cw = root.clientWidth  || window.innerWidth;
    var ch = root.clientHeight || window.innerHeight;
  ...
  function _pvFrame(ts) {
    if (!_pvActive) return;
    var dt = _pvLast ? Math.min((ts - _pvLast) / 1000, 0.05) : 0;
    _pvLast = ts;
    _pvT += dt;

    var d = _pvSizeCanvas();
    var cw = d.cw, ch = d.ch;
    var scale = Math.min(cw, ch) / 512;

    // Dark earth-tone background
    ctx.fillStyle = '#2a1a0a';
    ctx.fillRect(0, 0, cw, ch);

    // Soil vignette — radial dark overlay
    var soilGrad = ctx.createRadialGradient(cw*0.5, ch*0.5, 0, cw*0.5, ch*0.5, Math.max(cw, ch)*0.7);
    soilGrad.addColorStop(0,   'rgba(61,37,16,0)');
    soilGrad.addColorStop(1,   'rgba(26,10,0,0.55)');
    ctx.fillStyle = soilGrad;
    ctx.fillRect(0, 0, cw, ch);

    // Falling trash items — tile vertically
    var totalFall = _pvT * _pvFallPx;
    for (var ii = 0; ii < _pvItems.length; ii++) {
      var item = _pvItems[ii];
      var rawY = ((item.yOff + totalFall) % _pvTileH + _pvTileH) % _pvTileH;
      var sx = (item.x / 512) * cw;
      var sy = (rawY / 512) * ch;
      var sr = item.r * scale;
      ...
      ctx.translate(sx, sy);
      ctx.rotate(item.rot);
      drawTrashChunk(ctx, item.name, sr, 1.0);
      ctx.restore();

      // Wrapped copy
      var wrapY = rawY - _pvTileH;
      var wy = (wrapY / 512) * ch;
      ctx.save();
      ctx.translate(sx, wy);
      ctx.rotate(item.rot);
    ...
    var vig = ctx.createRadialGradient(cw*0.5, ch*0.5, Math.min(cw,ch)*0.2, cw*0.5, ch*0.5, Math.max(cw,ch)*0.72);
    vig.addColorStop(0.35, 'rgba(0,0,0,0)');
    vig.addColorStop(1,    'rgba(0,0,0,0.72)');
    ctx.fillStyle = vig;
    ctx.fillRect(0, 0, cw, ch);

    // Warm amber glow pulse (6-second breath cycle)
    var glow = 0.28 + Math.sin(_pvT * (Math.PI * 2 / 6)) * 0.12;
    var glowSc = 1 + Math.sin(_pvT * (Math.PI * 2 / 6)) * 0.032;
    var glowX = cw * 0.5, glowY = ch * 0.5;
    var glowRx = cw * 0.3 * glowSc, glowRy = ch * 0.28 * glowSc;
    var glowGrad = ctx.createRadialGradient(glowX, glowY, 0, glowX, glowY, Math.max(glowRx, glowRy));
    glowGrad.addColorStop(0,    'rgba(212,160,96,' + glow.toFixed(3) + ')');
    glowGrad.addColorStop(0.6,  'rgba(192,120,32,' + (glow*0.45).toFixed(3) + ')');
    glowGrad.addColorStop(1,    'rgba(128,64,16,0)');
    ctx.fillStyle = glowGrad;
    ctx.beginPath();
    ctx.ellipse(glowX, glowY, glowRx, glowRy, 0, 0, Math.PI*2);
    ctx.fill();

    // Worm icon — centred, 50% of min dimension, gentle bob
    var iconSz = Math.min(cw, ch) * 0.5;
    var iconBob = Math.sin(_pvT * (Math.PI * 2 / 6)) * iconSz * 0.015;
    if (window._pvIconImg && window._pvIconImg.complete) {
      ctx.save();
      ctx.globalAlpha = 0.97;
      ctx.drawImage(window._pvIconImg, (cw - iconSz)*0.5, (ch - iconSz)*0.5 + iconBob, iconSz, iconSz);
      ctx.restore();
    }

    requestAnimationFrame(_pvFrame);
  }

  // Preload the worm icon
  window._pvIconImg = new Image();
  window._pvIconImg.src = 'icon.png';

  // Expose stop function — called by setup() to hand off to the game loop
  window.stopPreview = function() { _pvActive = false; };
